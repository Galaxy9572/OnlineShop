...
//生成表格
function readyToSendTable(pageNumber, pageSize) {
    var url = "../user/queryReadyToSend"; //请求的URL
    var reqParams = {
        'pageNumber': pageNumber,
        'pageSize': pageSize
    };//请求数据
            ...
            success: function (data) {
                if (data.isError === false) {
                    ...
                    var newoptions = {
                        ...
                        onPageClicked: function (e, originalEvent, type, page) {
                            readyToSendTable(page, PAGESIZE);//默认每页最多10条
                        }
                    };
                    $('#readyToSendPaginator').bootstrapPaginator("setOptions", newoptions); //重新设置总页面数目
                    var dataList = data.dataList;
                    $("#readyToSendTableBody").empty();//清空表格内容
                    if (dataList.length > 0) {
                        $(dataList).each(function () {//重新生成
                            $("#readyToSendTableBody").append('<tr>')
                        ...
                        $("#readyToSendTableBody").append('<tr><th colspan ="6"><center>暂无消息</center></th></tr>');
                    }
                } else {
                    alert(data.errorMsg);
                }
...
$(function () {
    ...
    //生成底部分页栏
    $('#readyToSendPaginator').bootstrapPaginator(readyToSendTableOptions);
    readyToSendTable(1, 10);//默认空白查全部
    //创建结算规则
    $("#bt_readyToSendQuerySubmit").bind("click", function () {
        readyToSendTable(1, PAGESIZE);
    });
    ...
    var userId = $("#userId").val();
    var statement = $("#select_statement").find("option:selected").val();
    msgTable(userId, statement, 1, 10);//默认空白查全部
    //创建结算规则
